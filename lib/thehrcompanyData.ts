...
// Reuse the same Supabase client instance workenvoData.ts already creates -- both accounts
// hit the exact same project with the exact same anon key, so a second createClient() call
// here would just be a second GoTrueClient listening on the same localStorage auth key
// (Supabase's own SDK warns loudly about this "Multiple GoTrueClient instances" case).
import { supabaseBrowser } from "@/lib/workenvoData";

// Mirrors lib/workenvoData.ts function-for-function -- The HR Company is a second real
// Supabase-backed client on the exact same schema (contacts/email_messages/prospect_feed/
// config/run_requests are all client_id-scoped by design), not a different data shape. The
// only thing that differs between the two files is which client_id every query is scoped to.
// Deliberately a separate file rather than a parameterized shared module -- matches this
// project's "simple per-client branches, not a generic framework" approach used throughout
// the backend (see CLAUDE.md).
...
// Known id of the 'thehrcompany' row in the `clients` table (seeded 2026-07-22, see
// CLAUDE.md's "Second client onboarding" section). Hardcoded rather than looked up: `clients`
// has no public RLS read policy (by design -- only `contacts`/`emails`/`notifications`/`runs`
// are meant for anon-key dashboard reads).
const THEHRCOMPANY_CLIENT_ID = "f777e9e0-8bc6-41d7-ab69-2ef08c7329b3";
...
async function fetchIcpHealth(): Promise<{ pct: number | null; note: string | null }> {
  const { data, error } = await supabaseBrowser
    ...
    .eq("client_id", THEHRCOMPANY_CLIENT_ID)
    .not("icp_health", "is", null)
    .order("started_at", { ascending: true });

  if (error) {
    console.error("[thehrcompanyData] fetchIcpHealth failed:", error.message);
...
export async function fetchProspectsPage(args: {
  ...
  if (error) {
    console.error("[thehrcompanyData] fetchProspectsPage failed:", error.message);
...
export async function fetchAllProspectsLean(): Promise<ProspectListItem[]> {
  ...
  if (error) {
    console.error("[thehrcompanyData] fetchAllProspectsLean failed:", error.message);
    return [];
  }
...
export async function fetchLatestProspectId(args: {
  ...
  if (error) {
    console.error("[thehrcompanyData] fetchLatestProspectId failed:", error.message);
    return null;
  }
...
export async function fetchProspectById(contactId: string): Promise<Prospect | null> {
  ...
  if (emailResult.error) {
    console.error("[thehrcompanyData] fetchProspectById failed:", emailResult.error.message);
    return null;
  }
  if (!emailResult.data) return null;
  if (followUpResult.error) {
    console.error("[thehrcompanyData] follow-up check failed:", followUpResult.error.message);
  }
...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  ...
  if (statusResult.error) {
    console.error("[thehrcompanyData] fetchDashboardStats failed:", statusResult.error.message);
  }
...
export async function fetchRecentActivity(): Promise<ActivityEvent[]> {
  ...
  if (error) {
    console.error("[thehrcompanyData] fetchRecentActivity failed:", error.message);
    return [];
  }
...
export async function fetchThreadMessages(contactId: string): Promise<ThreadMessage[]> {
  const { data, error } = await supabaseBrowser
    ...
    .eq("client_id", THEHRCOMPANY_CLIENT_ID);
  if (error) {
    console.error("[thehrcompanyData] fetchThreadMessages failed:", error.message);
    return [];
  }
  return (data ?? [])
    .map((row) => ({
...
// Flips a message to 'approved' — a DB trigger fires the appropriate send Edge Function the
// instant this transition happens. Note: no Outlook send function exists yet for this client
// (wk-send-email now explicitly refuses any client whose send_provider isn't "gmail" -- see
// CLAUDE.md) -- clicking "Send Now" here will currently fail loudly rather than silently do
// nothing, which is the deliberately safe behavior until Outlook sending is built.
...
export async function fetchMessageStatus(messageId: string): Promise<string | null> {
  const { data, error } = await supabaseBrowser
    ...
    .maybeSingle();
  if (error) {
    console.error("[thehrcompanyData] fetchMessageStatus failed:", error.message);
    return null;
  }
...
export async function fetchSenderOptions(): Promise<SenderOption[]> {
  const { data, error } = await supabaseBrowser
    ...
    .maybeSingle();
  if (error) {
    console.error("[thehrcompanyData] fetchSenderOptions failed:", error.message);
    return [];
  }
...
export async function fetchAutoSend(): Promise<boolean> {
  const { data, error } = await supabaseBrowser
    ...
    .maybeSingle();
  if (error) {
    console.error("[thehrcompanyData] fetchAutoSend failed:", error.message);
    return false;
  }
...
// per-email send_from, so wk-send-email always falls back to this). Empty today --
// no sender_email config row exists for this client yet (Outlook sending isn't built).
export async function fetchDefaultSender(): Promise<string> {
  const { data, error } = await supabaseBrowser
    ...
    .maybeSingle();
  if (error) {
    console.error("[thehrcompanyData] fetchDefaultSender failed:", error.message);
    return "";
  }
...
export async function fetchContactNotes(contactId: string): Promise<ContactNote[]> {
  const { data, error } = await supabaseBrowser
    ...
    .order("created_at", { ascending: false });
  if (error) {
    console.error("[thehrcompanyData] fetchContactNotes failed:", error.message);
    return [];
  }
  return (data ?? []).map((row) => ({
    id: row.id,
...
export async function addContactNote(
  ...
  const { error } = await supabaseBrowser
    .from("contact_notes")
    .insert({ contact_id: contactId, client_id: THEHRCOMPANY_CLIENT_ID, author, text });
  if (error) throw new Error(error.message);
}
...
// The thehrcompany-scheduler's poll loop picks this up from the table instead.
export async function enqueueRun(count: number): Promise<void> {
  const { error } = await supabaseBrowser
    .from("run_requests")
    .insert({ client_id: THEHRCOMPANY_CLIENT_ID, requested_count: count });
  if (error) throw new Error(error.message);
}
...
export async function fetchLatestRunRequest(): Promise<RunRequest | null> {
  ...
  if (error) {
    console.error("[thehrcompanyData] fetchLatestRunRequest failed:", error.message);
    return null;
  }
  return data;
}
